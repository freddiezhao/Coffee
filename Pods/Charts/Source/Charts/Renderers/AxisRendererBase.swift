...
open class AxisRendererBase: Renderer
{
    ...
    @objc open func computeAxisValues(min: Double, max: Double)
    {
        ...
        //var interval = rawInterval.roundedToNextSignficant()//这个函数求给定数的数量级最接近的10^n数
        var interval = rawInterval//保持原值
        ...
        if intervalSigDigit > 5
        {
            ...
            //interval = floor(10.0 * Double(intervalMagnitude))//这里把间隔60变成了100
        }
        //NSLog("%f", interval)
